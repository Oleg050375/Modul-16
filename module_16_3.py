...
@app.get('/users')  # хэндлер обработки запроса списка пользователей
async def list_answer() -> dict:
    return users


@app.post('/user/{username}/{age}')  # хаэндлер обработки запроса на создание нового пользователя
...
@app.put('/user/{user_id}/{username}/{age}')  # хаэндлер обработки запроса на изменение записи
...
@app.delete('/user/{user_id}')  # хаэндлер обработки запроса на удаление записи
